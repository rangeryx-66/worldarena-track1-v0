...
QC_FIELDS = [
    ...
    "color_shift_score", "temporal_flicker_score",  # compatibility: now background-only scores
    "global_content_rgb_shift_score", "global_content_brightness_step", "foreground_area_change_score",
    "background_luma_flicker_score", "background_color_shift_score", "background_changed_area_ratio",
    "background_color_changed_area_ratio", "background_delta_sign_consistency", "background_color_direction_consistency",
    "background_mask_reliable", "background_reliable_pair_ratio", "illumination_flicker_candidate", "true_color_shift_candidate",
    "compression_artifact_score", "sharpness_laplacian",
    "robot_visible_ratio", "robot_near_object_ratio", "arm_visible_ratio", "left_gripper_visible_ratio",
    "right_gripper_visible_ratio", "end_effector_visible_ratio", "contact_region_visible_ratio",
    "object_motion_without_visible_contact_score", "object_like_motion_ratio", "object_motion_event_count",
    "bad_frame_ratio", "motion_score", "deterministic_hard_fail", "hard_fail_reason",
    "action_joint14_valid", "action_has_nan", "action_has_inf", "heuristic_candidate_labels",
    "rule_qc_context", "qc_status", "qc_reason",
]
...
def sample_dense_indices(frame_count: int, fps: float, target_fps: float = 6.0, max_frames: int = 96) -> list[int]:
    if frame_count <= 0:
        return []
    ...
    if len(idx) > max_frames:
        idx = sorted(set(int(round(x)) for x in np.linspace(0, len(idx) - 1, max_frames)))
...
def resize_for_metrics(frame_bgr: np.ndarray, max_width: int = 320) -> np.ndarray:
    ...
    return cv2.resize(frame_bgr, (max_width, max(1, int(round(h * scale)))), interpolation=cv2.INTER_AREA)
...
def compute_frame_masks(frame_bgr: np.ndarray, prev_frame_bgr: np.ndarray | None = None) -> dict[str, Any]:
    h, w = frame_bgr.shape[:2]
    ...
    edge_d = cv2.dilate(edges.astype(np.uint8), np.ones((5, 5), np.uint8), iterations=1).astype(bool)
    ...
    if prev_frame_bgr is not None:
        ...
        motion_mask = cv2.dilate(motion_mask.astype(np.uint8), np.ones((3, 3), np.uint8), iterations=1).astype(bool)
    ...
    foreground_mask = clean_mask((robot_mask | object_like_mask | edge_d | motion_mask) & ~background_mask, k=3, min_area=12)
...
def compute_global_content_metrics(frames: list[np.ndarray], masks: list[dict[str, Any]]) -> dict[str, float]:
    if not frames:
        return {"global_content_rgb_shift_score": 0.0, "global_content_brightness_step": 0.0, "foreground_area_change_score": 0.0}
    rgb_means = []
    luma = []
    fg = []
    ...
    rgb_step = float(np.max(np.linalg.norm(np.diff(rgb_arr, axis=0), axis=1))) if len(rgb_arr) >= 2 else 0.0
    luma_step = float(np.max(np.abs(np.diff(np.asarray(luma, dtype=np.float32))))) if len(luma) >= 2 else 0.0
    fg_step = float(np.max(np.abs(np.diff(np.asarray(fg, dtype=np.float32))))) if len(fg) >= 2 else 0.0
    return {
...
def compute_background_flicker_metrics(frames: list[np.ndarray], masks: list[dict[str, Any]]) -> dict[str, Any]:
    deltas = []
    changed = []
    sign_cons = []
    fg_delta = []
    ...
    for i in range(1, len(frames)):
        ...
        if not (prev["mask_reliability"] and cur["mask_reliability"] and common_frac > 0.12 and common_bg.sum() > 900):
            continue
        ...
        fg_delta.append(abs(float(cur["foreground_fraction"]) - float(prev["foreground_fraction"])))
        reliable_pairs += 1
    ...
    candidate = bool(bg_reliable and changed_score > 0.55 and score > 8.0 and sign_score > 0.75 and fg_score < 0.10)
    return {
...
def compute_background_color_shift_metrics(frames: list[np.ndarray], masks: list[dict[str, Any]]) -> dict[str, Any]:
    scores = []
    changed = []
    direction_cons = []
    reliable_pairs = 0
    ...
    bg_reliable = reliable_pairs >= max(2, int(0.35 * max(pair_count, 1)))
    candidate = bool(bg_reliable and score > 6.0 and changed_score > 0.45 and direction_score > 0.70)
    return {
...
def compute_contact_motion_metrics(frames: list[np.ndarray], masks: list[dict[str, Any]]) -> dict[str, Any]:
    robot_visible = []
    robot_near_object = []
    contact_visible = []
    ...
    for i, m in enumerate(masks):
        ...
        obj_region = cv2.dilate((obj | prev_obj).astype(np.uint8), np.ones((7, 7), np.uint8), iterations=1).astype(bool)
        object_motion = motion & obj_region & ~cv2.dilate(robot.astype(np.uint8), np.ones((5, 5), np.uint8), iterations=1).astype(bool)
        ...
        if object_motion_frac > 0.0012 and object_motion.sum() > 28:
            ...
            near_motion = mask_near(object_motion, robot | masks[i - 1]["robot_mask"], radius=18)
            if not near_motion:
                no_contact_events += 1
                current_run += 1
            ...
            current_run = 0
    event_ratio = float(no_contact_events / max(object_motion_events, 1)) if object_motion_events else 0.0
    object_motion_ratio = float(object_motion_events / max(len(frames) - 1, 1)) if len(frames) > 1 else 0.0
    score = event_ratio if object_motion_events >= 2 and sustained_no_contact >= 2 else 0.0
    return {
        "robot_visible_ratio": float(np.mean(robot_visible)) if robot_visible else 0.0,
        "robot_near_object_ratio": float(np.mean(robot_near_object)) if robot_near_object else 0.0,
        "contact_region_visible_ratio": float(np.mean(contact_visible)) if contact_visible else 0.0,
...
def read_video_frames(video_path: str, indices: list[int], resize_width: int = 320) -> tuple[list[np.ndarray], int]:
...
def analyze_video(args_tuple: tuple[int, dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
    ...
    dense_indices = sample_dense_indices(frame_count, fps, dense_target_fps, max_dense_frames)
    ...
    bg_stable = flicker_metrics["background_luma_flicker_score"] < 6 and color_metrics["background_color_shift_score"] < 5
    ...
    elif contact_metrics["object_like_motion_ratio"] > 0.12 and contact_metrics["contact_region_visible_ratio"] < 0.12:
        labels.append("contact_visibility_borderline")
    if contact_metrics["robot_visible_ratio"] < 0.08 and contact_metrics["object_like_motion_ratio"] > 0.08:
    ...
    elif any(x in DPO_LABELS for x in labels) and contact_metrics["object_motion_without_visible_contact_score"] > 0.80:
    ...
    left_ratio = float(np.mean([m["robot_mask"][:, : m["robot_mask"].shape[1] // 2].mean() > 0.004 for m in masks])) if masks else 0.0
    right_ratio = float(np.mean([m["robot_mask"][:, m["robot_mask"].shape[1] // 2 :].mean() > 0.004 for m in masks])) if masks else 0.0
    rule_context = {
        "hard_fail": deterministic_hard_fail,
        "global_content_change": {
            "global_content_rgb_shift_score": global_metrics["global_content_rgb_shift_score"],
            "global_content_brightness_step": global_metrics["global_content_brightness_step"],
            "foreground_area_change_score": global_metrics["foreground_area_change_score"],
        },
        "background_artifact_metrics": {
            "background_luma_flicker_score": flicker_metrics["background_luma_flicker_score"],
            "background_color_shift_score": color_metrics["background_color_shift_score"],
            "background_changed_area_ratio": flicker_metrics["background_changed_area_ratio"],
            "background_color_changed_area_ratio": color_metrics["background_color_changed_area_ratio"],
            "background_delta_sign_consistency": flicker_metrics["background_delta_sign_consistency"],
            "background_color_direction_consistency": color_metrics["background_color_direction_consistency"],
        },
        ...
        "visibility_metrics": {
            ...
            "contact_region_visible_ratio": contact_metrics["contact_region_visible_ratio"],
        },
        "metric_reliability": {
            "background_mask_reliable": flicker_metrics["background_mask_reliable"],
            "background_reliable_pair_ratio": flicker_metrics["background_reliable_pair_ratio"],
            "background_color_reliable_pair_ratio": color_metrics["background_color_reliable_pair_ratio"],
        },
        "notes": [
    ...
    out.update({
        ...
        "background_mask_reliable": flicker_metrics["background_mask_reliable"],
        "background_reliable_pair_ratio": flicker_metrics["background_reliable_pair_ratio"],
        "illumination_flicker_candidate": flicker_metrics["illumination_flicker_candidate"],
        ...
        "contact_region_visible_ratio": contact_metrics["contact_region_visible_ratio"],
        "object_motion_without_visible_contact_score": contact_metrics["object_motion_without_visible_contact_score"],
...
def write_report(out_dir: Path, manifest_path: Path, full: pd.DataFrame, qc: pd.DataFrame, version: str) -> None:
    ...
    metrics = [
        "global_content_rgb_shift_score", "global_content_brightness_step", "foreground_area_change_score",
        "background_luma_flicker_score", "background_color_shift_score", "background_changed_area_ratio",
        "background_delta_sign_consistency", "robot_visible_ratio", "robot_near_object_ratio",
        "contact_region_visible_ratio", "object_motion_without_visible_contact_score", "object_like_motion_ratio",
...
def write_outputs(out_dir: Path, manifest_path: Path, manifest: pd.DataFrame, qc: pd.DataFrame, version: str) -> None:
    full = manifest.copy()
    ...
    full[full["qc_status"] == "pass"].to_parquet(out_dir / f"episode_manifest_qc_pass{suffix}.parquet", index=False)
    full[full["qc_status"] == "warn"].to_parquet(out_dir / f"episode_manifest_qc_warn{suffix}.parquet", index=False)
    full[full["qc_status"] == "reject"].to_parquet(out_dir / f"episode_manifest_qc_reject{suffix}.parquet", index=False)
    full[full["qc_status"] == "dpo_loser_candidate"].to_csv(out_dir / "dpo_loser_candidates_rule_v2.csv", index=False)
    cs = out_dir / "contact_sheets"
    cs.mkdir(parents=True, exist_ok=True)
    filters = {
        "background_flicker_candidate": full[full["heuristic_candidate_labels"].fillna("").str.contains("background_flicker_candidate", regex=False)],
        "true_color_shift_candidate": full[full["heuristic_candidate_labels"].fillna("").str.contains("true_color_shift_candidate", regex=False)],
        "foreground_entry_caused_global_shift": full[full["heuristic_candidate_labels"].fillna("").str.contains("foreground_entry_caused_global_shift", regex=False)],
        "object_motion_without_contact_candidate": full[full["heuristic_candidate_labels"].fillna("").str.contains("possible_object_motion_without_contact", regex=False)],
        "contact_visibility_borderline": full[full["heuristic_candidate_labels"].fillna("").str.contains("contact_visibility_borderline", regex=False)],
        "pass_high_global_shift_but_background_stable": full[(full["qc_status"] == "pass") & ((full["global_content_rgb_shift_score"] > 35) | (full["global_content_brightness_step"] > 18)) & (full["background_luma_flicker_score"] < 6) & (full["background_color_shift_score"] < 5)],
...
def progress_iter(iterator, total: int, enabled: bool = True, interval: int = 25):
    ...
    for i, item in enumerate(iterator, 1):
        ...
        if i == total or i - last >= max(1, interval):
            ...
            print(f"video-qc {i}/{total} ({i/total:.1%}) rate={rate:.2f}/s eta={eta/60:.1f}min", flush=True)
            last = i
...
def main() -> None:
    ...
    ap.add_argument("--max-sampled-frames", type=int, default=16, help="Legacy alias; v2 uses dense sampling.")
    ...
    if not args.manifest or not args.out:
        ap.error("--manifest and --out are required unless --run-regression-tests is used")
    ...
    if args.workers <= 1:
        qc_rows = list(progress_iter((analyze_video(r) for r in rows), len(rows), progress_enabled, args.progress_interval))
    else:
        with Pool(processes=args.workers) as pool:
            qc_rows = list(progress_iter(pool.imap_unordered(analyze_video, rows, chunksize=8), len(rows), progress_enabled, args.progress_interval))
