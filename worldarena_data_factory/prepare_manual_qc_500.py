...
def load_current_qc(dataset_root: Path) -> pd.DataFrame:
    ...
    rows = []
    by_source = {
        s["_qc_source"].iloc[0]: s.set_index(s["episode_id"].astype(str), drop=False)
        for s in sources
    }
    for eid in all_ids:
        ...
        rows.append(
            {
                "episode_id": eid,
                "current_qc_status": status or "unknown",
                "current_qc_reason": reason,
                "current_qc_labels": labels,
                "risk_summary": " | ".join(risk)[:1600],
            }
        )
...
def sample_stratified(df: pd.DataFrame, n: int, seed: int) -> pd.DataFrame:
    ...
    work = df.copy()
    work["_T_bin"] = pd.qcut(
        pd.to_numeric(work["T"], errors="coerce").fillna(0).rank(method="first"),
        q=min(4, len(work)),
        labels=False,
        duplicates="drop",
    )
    work["_C_bin"] = pd.qcut(
        pd.to_numeric(work["action_complexity_score"], errors="coerce")
        .fillna(0)
        .rank(method="first"),
        q=min(4, len(work)),
        labels=False,
    ...
    while groups and len(selected) < min(n, len(work)):
        ...
        for _, g in groups:
            ...
            if remaining.empty:
                continue
            selected.append(
                remaining.sample(1, random_state=int(rng.integers(0, 1_000_000))).index[
                    0
                ]
            )
    ...
    if len(selected) < min(n, len(work)):
        rest = (
            work.drop(index=selected)
            .sample(
                min(n - len(selected), len(work) - len(selected)), random_state=seed
            )
            .index.tolist()
        )
        selected += rest
...
def hard_score(df: pd.DataFrame) -> pd.Series:
    status = (
        df.get("current_qc_status", pd.Series(["unknown"] * len(df)))
        .fillna("")
        .astype(str)
        .str.upper()
    )
    score = (
        status.isin(
            ["WARN", "REJECT", "DPO_LOSER", "DPO_LOSER_CANDIDATE", "NEED_HUMAN_REVIEW"]
        ).astype(float)
        * 5
    )
    ...
    risk = (
        df.get("risk_summary", pd.Series([""] * len(df)))
        .fillna("")
        .astype(str)
        .str.lower()
    )
    for token, w in [
        ...
        ("low_action", 1.5),
        ("hard_fail", 2.0),
        ("contact_region_visible_ratio=0", 1.0),
        ("robot_visible_ratio=0", 1.0),
    ]:
...
def draw_sheet(
    ...
    d.text((8, 8), title, fill=(0, 0, 0), font=font)
...
def make_overview(video: Path, out_path: Path, n: int = 12):
    ...
    if cap.isOpened():
        ...
        for idx in (
            np.linspace(0, max(0, count - 1), min(n, max(1, count)))
            .astype(int)
            .tolist()
            if count
            else []
            ...
            if f is not None:
                frames.append((idx, idx / fps, f))
    cap.release()
    draw_sheet(frames, out_path, "overview")
...
def make_motion_peak(video: Path, out_path: Path, n: int = 12):
    ...
    if cap.isOpened():
        ...
        count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
        sample = (
            np.linspace(1, max(1, count - 1), min(80, max(1, count - 1)))
            .astype(int)
            .tolist()
            if count > 1
            else []
        ...
        for idx in sample:
            ...
            if prev is not None:
                peaks.append((float(cv2.absdiff(g, prev).mean()), idx))
            prev = g
        chosen = []
        ...
        for idx in sorted(set(chosen))[:n]:
            ...
            if f is not None:
                frames.append((idx, idx / fps, f))
    cap.release()
    draw_sheet(frames, out_path, "motion peaks")


def action_peak_indices(row: pd.Series, n: int = 12) -> list[int]:
    path = Path(
        safe_str(
            row.get("action_joint14_raw_path") or row.get("action_joint14_norm_path")
        )
    )
    ...
    try:
        ...
        return sorted(
            set(int(round((p / max(T - 1, 1)) * (frame_count - 1))) for p in peaks)
        )[:n]
    except Exception:
        return []
    ...
    if cap.isOpened():
        ...
        for idx in action_peak_indices(row, n):
            ...
            if f is not None:
                frames.append((idx, idx / fps, f))
    cap.release()
    draw_sheet(frames, out_path, "action peaks")
...
def preserve_existing_annotations(
    new_df: pd.DataFrame, out_dir: Path, csv_path: Path
) -> pd.DataFrame:
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=10,
        help="Print progress every N prepared samples.",
    )
    args = ap.parse_args()
    out = args.out
    ...
    if csv_path.exists() and not args.force_regenerate:
        print(csv_path)
        print(
            f"exists rows={len(pd.read_csv(csv_path))}; use --force-regenerate to resample"
        )
        return
    ...
    random_part = sample_stratified(df, random_n, args.seed).copy()
    random_part["sample_group"] = "random_eval"
    rest = df.drop(index=random_part.index, errors="ignore").copy()
    rest["_hard_score"] = hard_score(rest)
    hard_part = (
        rest.sort_values("_hard_score", ascending=False)
        .head(hard_n)
        .drop(columns=["_hard_score"], errors="ignore")
        .copy()
    )
    hard_part["sample_group"] = "hard_dev"
    selected = pd.concat([random_part, hard_part], ignore_index=True)
    if len(selected) < n_total:
        fill = (
            df.drop(index=selected.index, errors="ignore")
            .sample(
                min(n_total - len(selected), len(df) - len(selected)),
                random_state=args.seed,
            )
            .copy()
        )
        fill["sample_group"] = "random_eval"
    ...
    total = len(selected)
    print(
        f"preparing {total} samples; generating overview/motion/action sheets...",
        flush=True,
    )
    for i, row in selected.iterrows():
        ...
        try:
            ...
            rows.append(
                {
                    "sample_id": f"mqc_{i:06d}",
                    "sample_group": row.get("sample_group", ""),
                    "episode_id": eid,
                    "video_path": vpath,
                    "overview_sheet": str(overview),
                    "motion_peak_sheet": str(motion),
                    "action_peak_sheet": str(action),
                    "first_frame_path": safe_str(row.get("first_frame_320x240_path")),
                    "task_family": safe_str(row.get("task_family")),
                    "robotwin_task_name": safe_str(row.get("robotwin_task_name")),
                    "prompt_short": safe_str(
                        row.get("prompt_short") or row.get("prompt_worldarena_style")
                    ),
                    "T": safe_str(row.get("T")),
                    "action_complexity_score": safe_str(
                        row.get("action_complexity_score")
                    ),
                    "dominant_arm": safe_str(row.get("dominant_arm")),
                    "current_qc_status": safe_str(row.get("current_qc_status")),
                    "current_qc_reason": safe_str(row.get("current_qc_reason")),
                    "current_qc_labels": safe_str(row.get("current_qc_labels")),
                    "risk_summary": safe_str(row.get("risk_summary")),
                    "human_label": "",
                    "human_reason": "",
                    "human_confidence": "",
                    "notes": "",
        ...
        if (
            ...
            or done == total
            or (args.progress_every > 0 and done % args.progress_every == 0)
        ):
            print(
                f"prepared {done}/{total} samples; rows={len(rows)} errors={len(errors)}",
                flush=True,
            )
