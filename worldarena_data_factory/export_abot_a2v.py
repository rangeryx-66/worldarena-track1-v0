...
from utils import ensure_dirs, read_table, write_jsonl, write_json, is_v0_training_embodiment
...
def load_rows(out: Path, manifest_arg: str | None, qc_source: str, vlm_qc_dir: str | None):
    if qc_source == 'vlm':
        qc_dir = Path(vlm_qc_dir) if vlm_qc_dir else out / 'v0_1_qc_vlm'
        path = qc_dir / 'vlm_qc_scores.csv'
        if not path.exists():
            raise FileNotFoundError(f'VLM QC scores not found: {path}. Run merge_qc_with_vlm.py first.')
        return read_table(path)
    return read_table(Path(manifest_arg) if manifest_arg else out / 'manifests' / 'episode_manifest.parquet')


def base_episode_ok(r, out, secondary_embodiment, include_secondary):
    if str(r.get('success')).lower() != 'true' or not r.get('video_640x480_path'):
        return False
    if not is_v0_training_embodiment(r.get('embodiment', ''), 'aloha-agilex', secondary_embodiment, out, include_secondary):
        return False
...
def export(rows, out, name, field, secondary_embodiment=None, include_secondary=False):
    data = []
    for r in rows:
        if not base_episode_ok(r, out, secondary_embodiment, include_secondary) or not r.get(field):
            continue
...
def export_vlm(rows, out, secondary_embodiment=None, include_secondary=False):
    ...
    for r in rows:
        ...
        if r.get('action_ee16_raw_path') and (final_decision == 'PASS' or (final_decision == 'WARN_KEEP' and a2v_score >= 1)):
...
def main():
    ...
    if args.qc_source == 'vlm':
        n_a2v, n_dpo = export_vlm(rows, out, args.secondary_embodiment, args.include_secondary_embodiment)
        print(f'a2v_ee16_v0_1_vlm={n_a2v} dpo_loser_bank={n_dpo}')
    else:
        n1 = export(rows, out, 'a2v_worldarena_joint14', 'action_joint14_norm_path', args.secondary_embodiment, args.include_secondary_embodiment)
        n2 = export(rows, out, 'a2v_worldarena_ee16', 'action_ee16_raw_path', args.secondary_embodiment, args.include_secondary_embodiment)
        n3 = export(rows, out, 'a2v_worldarena_joint14_ee16', 'action_joint14_ee16_raw_path', args.secondary_embodiment, args.include_secondary_embodiment)
