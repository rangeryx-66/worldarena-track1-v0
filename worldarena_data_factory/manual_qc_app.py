...
REASONS = [
    "good",
    "robot_or_gripper_not_visible",
    "contact_not_visible",
    "object_moves_without_contact",
    "no_task_progress",
    "action_video_mismatch",
    "severe_flicker_or_exposure_jump",
    "broken_or_black_frames",
    "bad_compression_or_noise",
    "wrong_domain_or_wrong_robot",
    "severe_physics_issue",
    "ambiguous",
]
REASON_LABELS_ZH = {
    "good": "质量好 / 可用",
...
def merge_existing_annotations(
    base_df: pd.DataFrame, labeled_df: pd.DataFrame
) -> pd.DataFrame:
...
def load_data(base: Path, out: Path) -> pd.DataFrame:
    ...
    if "episode_id" in base_df.columns and "episode_id" in labeled_df.columns:
        if (
            base_df["episode_id"].astype(str).tolist()
            != labeled_df["episode_id"].astype(str).tolist()
        ):
            return merge_existing_annotations(base_df, labeled_df)
    return ensure_annotation_columns(labeled_df)
...
def first_unlabeled_index(
    df: pd.DataFrame, indices: list[int], start_after: int = -1
) -> int:
    for idx in indices:
...
""",
    ...
    if should_reload:
        ...
        if old_idx < len(st.session_state.df):
            ...
            st.session_state.idx = first_unlabeled_index(
                st.session_state.df, list(range(len(st.session_state.df)))
            )
    df = st.session_state.df

    st.sidebar.header("Filters")
    only_unlabeled = st.sidebar.checkbox("show only unlabeled", value=False)
    sample_groups = ["all"] + sorted(
        [
            x
            for x in df.get("sample_group", pd.Series(dtype=str)).astype(str).unique()
            if x
        ]
    )
    ...
    tasks = ["all"] + sorted(
        [
            x
            for x in df.get("task_family", pd.Series(dtype=str)).astype(str).unique()
            if x
        ]
    )
    ...
    qcs = ["all"] + sorted(
        [
            x
            for x in df.get("current_qc_status", pd.Series(dtype=str))
            .astype(str)
            .unique()
            if x
        ]
    ...
    if only_unlabeled:
        mask &= df["human_label"].astype(str).eq("")
    if group != "all":
        mask &= df["sample_group"].astype(str).eq(group)
    if task != "all":
        mask &= df["task_family"].astype(str).eq(task)
    if qc != "all":
        mask &= df["current_qc_status"].astype(str).eq(qc)
    if hl == "unlabeled":
        mask &= df["human_label"].astype(str).eq("")
    elif hl != "all":
        mask &= df["human_label"].astype(str).eq(hl)
    if hc != "all":
        mask &= df["human_confidence"].astype(str).eq(hc)
    filtered = df[mask]
    ...
    if j2.button("Jump") and jump_text.strip():
        m = df[
            (df["sample_id"].astype(str) == jump_text.strip())
            | (df["episode_id"].astype(str) == jump_text.strip())
        ]
        if not m.empty:
            st.session_state.idx = int(m.index[0])
            st.rerun()
        else:
            st.warning("not found")
    if j3.button("Next unlabeled"):
        st.session_state.idx = first_unlabeled_index(df, indices, st.session_state.idx)
        st.rerun()

    default_label = (
        str(current.get("human_label", ""))
        if str(current.get("human_label", "")) in ["PASS", "REJECT"]
        else "REJECT"
    )
    existing_reasons = [
        x
        for x in str(current.get("human_reason", ""))
        .replace("|", ",")
        .replace(";", ",")
        .split(",")
        if x in REASONS
    ...
    conf_val = (
        str(current.get("human_confidence", ""))
        if str(current.get("human_confidence", "")) in ["1", "2", "3"]
        else "2"
    )
    ...
    with meta_col:
        ...
        st.markdown(
            f"**{current.get('task_family','')}** / `{current.get('robotwin_task_name','')}`"
        )
        st.caption(
        ...
        st.text_area(
            "Prompt", str(current.get("prompt_short", "")), height=58, disabled=True
        )
        st.text_area(
            "Risk", str(current.get("risk_summary", "")), height=82, disabled=True
        )
        ...
        label = st.radio(
            "human_label",
            ["PASS", "REJECT"],
            index=["PASS", "REJECT"].index(default_label),
            horizontal=True,
        ...
        reason = st.multiselect(
            ...
            format_func=lambda x: REASON_LABELS_ZH.get(x, x),
            label_visibility="collapsed",
        )
        confidence = st.select_slider(
            ...
            options=["1", "2", "3"],
            value=conf_val,
            format_func=lambda x: {"1": "1 不确定", "2": "2 基本确定", "3": "3 很确定"}[
                x
            ],
            label_visibility="collapsed",
        )
        ...
        annotator = st.text_input(
            "annotator",
            value=str(current.get("annotator", "")) or "default",
            label_visibility="collapsed",
        )
    ...
    with media_col:
        ...
        if video_path.exists():
            show_video_compact(st, video_path, width=430)
        else:
            st.error(f'video missing: {current.get("video_path","")}')
        fp = Path(str(current.get("first_frame_path", "")))
        if fp.exists():
            st.image(str(fp), caption="first frame", width=210)

    with sheet_col:
        tabs = st.tabs(["Overview", "Motion Peak", "Action Peak"])
        for tab, col in zip(
            tabs, ["overview_sheet", "motion_peak_sheet", "action_peak_sheet"]
        ):
            with tab:
                sheet_path = Path(str(current.get(col, "")))
                if sheet_path.exists():
                    st.image(str(sheet_path), width=590)
                else:
                    st.warning(f"missing {col}: {sheet_path}")
    ...
    with meta_col:
        ...
        if b1.button("Save & Next", type="primary", use_container_width=True):
            save_current()
            st.session_state.idx = first_unlabeled_index(
                df, indices, st.session_state.idx
            )
            st.rerun()
        ...
        if b3.button("Previous", use_container_width=True):
            pos = (
                indices.index(st.session_state.idx)
                if st.session_state.idx in indices
                else 0
            )
            st.session_state.idx = indices[max(0, pos - 1)]
            st.rerun()
        if b4.button("Next", use_container_width=True):
            ...
            st.session_state.idx = indices[min(len(indices) - 1, pos + 1)]
            st.rerun()
