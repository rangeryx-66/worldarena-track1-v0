...
def flatten_vlm(row: dict[str, Any]) -> dict[str, Any]:
    ...
    rec = (
        ...
        if isinstance(row.get("recommended_use"), dict)
        else {}
    )
    out = {
...
def vlm_obvious_visual_hard_reject(row: pd.Series) -> bool:
    if str(row.get("vlm_decision", "")).upper() != "REJECT":
        return False
    zero_score_keys = [
        "score_robot_visibility",
        "score_gripper_contact_visibility",
        "score_object_visibility",
        "score_physical_plausibility",
        "score_temporal_consistency",
        "score_visual_quality",
        "score_task_relevance",
        "score_sft_positive_suitability",
        "score_a2v_positive_suitability",
    ]
    zero_count = 0
    ...
    hard_phrases = [
        "no robot",
        "robot or gripper is visible",
        "no gripper",
        "no object",
        "not visible in any frame",
        "blank",
        "gray",
        "no manipulation",
        "no visual content",
    ...
    hard_flags = any(
        str(row.get(k, "")).lower() == "true"
        for k in [
            "flag_wrong_robot_or_missing_robot",
            "flag_critical_contact_invisible",
            "flag_object_moves_without_contact",
        ]
    )
...
def vlm_visual_artifact_only(row: pd.Series) -> bool:
    ...
    try:
        ...
        semantic_ok = False
    bad_physical_flags = any(
        str(row.get(k, "")).lower() == "true"
        for k in [
            "flag_critical_contact_invisible",
            "flag_object_moves_without_contact",
            "flag_wrong_robot_or_missing_robot",
            "flag_prompt_action_mismatch",
        ]
    )
    visual_flag = (
        str(row.get("flag_severe_flicker_or_exposure_jump", "")).lower() == "true"
        or str(row.get("flag_severe_noise_or_compression", "")).lower() == "true"
    )
    evidence = str(row.get("vlm_evidence", "")).lower()
    mentions_only_visual = any(
        x in evidence
        for x in [
            "flicker",
            "color shift",
    ...
    return (
        semantic_ok and visual_flag and mentions_only_visual and not bad_physical_flags
    )
...
def decide(row: pd.Series) -> tuple[str, str, bool]:
    ...
    conflict = (strong_rule_reject and strong_vlm_pass) or (
        rule_status == "pass"
        and strong_vlm_reject
        and not obvious_visual_hard
        and not visual_artifact_only
    )
    if hard:
...
def report_text(df: pd.DataFrame, out: Path) -> str:
    ...
    matrix = pd.crosstab(
        df.get("qc_status", pd.Series(dtype=str)),
        df.get("vlm_decision", pd.Series(dtype=str)),
        dropna=False,
    )
    evidence = Counter()
    ...
    for _, row in df.iterrows():
        texts = [
            x.strip() for x in str(row.get("vlm_evidence", "")).split(";") if x.strip()
        ]
    ...
    lines += [
        "",
        "## Rule vs VLM Decision Matrix",
        "",
        matrix.to_markdown() if not matrix.empty else "No rows.",
        "",
        f'Conflict cases: {int(df.get("is_conflict", pd.Series(dtype=bool)).sum())}',
        "",
        "## Top Reject Reasons",
        "",
    ]
...
def main() -> None:
    ...
    rule = (
        pd.read_csv(args.rule_qc)
        if Path(args.rule_qc).exists()
        else pd.DataFrame(columns=["episode_id"])
    )
    ...
    write_table(
        out / "episode_manifest_qc_pass_vlm.parquet", df[df["final_decision"] == "PASS"]
    )
    write_table(
        out / "episode_manifest_qc_warn_vlm.parquet",
        df[df["final_decision"].isin(["WARN_KEEP", "NEED_HUMAN_REVIEW"])],
    )
    write_table(
        out / "episode_manifest_qc_reject_vlm.parquet",
        df[df["final_decision"] == "REJECT"],
    )
    df.to_csv(out / "vlm_qc_scores.csv", index=False)
    df[df["final_decision"] == "REJECT"].to_csv(
        out / "rejected_episodes_vlm.csv", index=False
    )
    df[df["final_decision"] == "DPO_LOSER_CANDIDATE"].to_csv(
        out / "dpo_loser_candidates_vlm.csv", index=False
    )
    ...
    for decision in [
        ...
        make_contact_sheet(
            df[df["final_decision"] == decision],
            out / "contact_sheets" / f"vlm_{decision}_samples.jpg",
            f"VLM {decision} samples",
        )
