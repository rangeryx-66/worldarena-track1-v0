...
def main():
    ...
    ap.add_argument(
        "--include-wrong-embodiment-dpo",
        action="store_true",
        help="Off by default for v0; wrong-embodiment negatives are v1/v2 robustness ablations.",
    )
    args = ap.parse_args()
    out = Path(args.out)
    ensure_dirs(out)
    ...
    if (
        not cross_path.exists()
        and (Path(args.analysis) / "cross_analysis.csv").exists()
    ):
        cross_path.write_text(
            (Path(args.analysis) / "cross_analysis.csv").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
    ...
    for r in cross:
        if len(pairs) >= args.target_pairs:
            break
        if (
            r.get("recommended_dpo_candidate_type")
            != "hard_negative_prompt_action_mismatch"
        ):
            continue
        rej.append(
            {
                ...
                "candidate_type": "prompt-action mismatch",
                "winner_domain": "aloha-agilex",
                "loser_domain": "aloha-agilex",
                "reason": "no winner/loser generated in v0 without rendered Aloha-AgileX RoboTwin success videos",
            }
        )
    policy = {
        ...
        "mix": {
            "prompt_action_mismatch": 150,
            "physics_contact_failure": 75,
            "wrong_object_target": 45,
            "temporal_unfinished": 30,
        },
    ...
    if args.include_wrong_embodiment_dpo:
        policy["embodiment_loser_types"] = [
            "wrong_embodiment_or_arm_count",
            "wrong_gripper_morphology",
            "left_right_arm_swap",
        ]
