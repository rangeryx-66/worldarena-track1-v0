...
def load_rows(out: Path, manifest_arg: str | None, qc_source: str, vlm_qc_dir: str | None):
    ...
    manifest_path = Path(manifest_arg) if manifest_arg else out / 'manifests' / 'episode_manifest.parquet'
...
def main():
    ...
    out_dir = out / ('sft_worldarena_style_v0_1_vlm' if args.qc_source == 'vlm' else 'sft_worldarena_style')
    data = []
    rng = random.Random(7)
    for r in rows:
        if str(r.get('success')).lower() != 'true' or not r.get('video_640x480_path'):
            continue
        if not is_v0_training_embodiment(r.get('embodiment', ''), 'aloha-agilex', args.secondary_embodiment, out, args.include_secondary_embodiment):
            continue
        if 'dual_arm_joint14_valid' not in str(r.get('quality_flags', '')):
            continue
        if args.qc_source == 'vlm' and str(r.get('final_decision', '')).upper() != 'PASS':
            continue
        x = rng.random()
        prompt = r.get('prompt_short') if x < .6 else (r.get('prompt_worldarena_style') if x < .85 else r.get('prompt_long_caption'))
