...
class Qwen3VLBackend(BaseVLMBackend):
    def __init__(self, model_path: str, mode: str = 'contact_sheet'):
        ...
        self.mode = mode
        if mode == 'video':
            raise RuntimeError('Qwen3-VL video mode is not enabled in this pipeline; use --mode contact_sheet for stable QC.')
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError('missing qwen_vl_utils; install with: python -m pip install qwen-vl-utils') from exc
        except Exception as exc:
            raise RuntimeError(
        ...
        self.model = ModelCls.from_pretrained(str(self.model_path), torch_dtype=dtype, device_map='auto')
    ...
    def generate(self, prompt: str, image_paths: list[Path], video_path: Path | None = None) -> str:
        ...
        with self.torch.no_grad():
            generated = self.model.generate(**inputs, max_new_tokens=768, do_sample=False)
...
def draw_labeled_sheet(frames: list[tuple[int, float, np.ndarray]], out_path: Path, title: str, cols: int = 4) -> None:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if not frames:
        img = Image.new('RGB', (640, 160), 'white')
        ImageDraw.Draw(img).text((20, 70), f'{title}: no readable frames', fill=(0, 0, 0))
...
def make_motion_sheet(video_path: Path, out_path: Path, num_frames: int) -> bool:
    ...
    if not indices:
        count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
        indices = np.linspace(0, max(0, count - 1), num_frames).astype(int).tolist() if count > 0 else []
    frames = []
    for idx in indices:
        arr = read_frame_at(cap, idx)
        if arr is not None:
...
def find_overlay_asset(row: pd.Series) -> Path | None:
    ...
    for name in ['condition_overlay.jpg', 'condition_map.jpg', 'trajectory_overlay.jpg', 'ee16_overlay.jpg']:
        p = episode_dir / name
        if p.exists():
            return p
    return None
...
def select_rows(manifest: pd.DataFrame, rule_qc: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    df = manifest.copy()
    if 'episode_id' not in df.columns:
        raise ValueError('manifest must contain episode_id')
    rule = rule_qc.copy()
    if not rule.empty and 'episode_id' in rule.columns:
        keep_cols = [c for c in rule.columns if c not in df.columns or c == 'episode_id']
        df = df.merge(rule[keep_cols], on='episode_id', how='left')
    if args.run_all:
        selected = df
    else:
        status = df.get('qc_status', pd.Series(['pass'] * len(df))).fillna('pass').astype(str).str.lower()
        labels = df.get('heuristic_candidate_labels', pd.Series([''] * len(df))).fillna('').astype(str)
        hard = df.get('deterministic_hard_fail', pd.Series([False] * len(df))).fillna(False).astype(str).str.lower().isin(['true', '1', 'yes'])
        ...
        if args.sample_pass_ratio > 0 and not pass_rows.empty:
            add_parts.append(pass_rows.sample(max(1, int(len(pass_rows) * args.sample_pass_ratio)), random_state=rng.randrange(10**9)))
        if args.sample_reject_ratio > 0 and not reject_rows.empty:
            add_parts.append(reject_rows.sample(max(1, int(len(reject_rows) * args.sample_reject_ratio)), random_state=rng.randrange(10**9)))
        if add_parts:
...
def run_backend_with_retries(backend: BaseVLMBackend, prompt: str, image_paths: list[Path], raw_path: Path, parse_error_path: Path, episode_id: str) -> dict[str, Any]:
    ...
    for attempt in range(1, 4):
        try:
            ...
            write_parse_error(parse_error_path, {'episode_id': episode_id, 'attempt': attempt, 'error': str(exc), 'raw_output_path': str(raw_path)})
            current_prompt = (
    ...
    return normalize_vlm_result({
        ...
        'confidence': 0.0,
        'evidence': ['VLM output failed strict JSON parsing after retries'] + errors[:2],
        'recommended_use': {'use_for_sft': False, 'use_for_a2v': False, 'use_for_dpo_winner': False, 'use_for_dpo_loser': False},
    })
...
def main() -> None:
    ...
    ap.add_argument('--mode', choices=['contact_sheet', 'video'], default='contact_sheet')
    ...
    rule_qc = pd.read_csv(args.rule_qc) if Path(args.rule_qc).exists() else pd.DataFrame()
    cfg = load_prompt_config(Path(args.prompt_config))
    rule_by_episode = {str(r.get('episode_id')): r for r in rule_qc.fillna('').to_dict('records')} if not rule_qc.empty else {}
    selected = select_rows(manifest, rule_qc, args)
    selected.to_csv(out / 'vlm_selected_episodes.csv', index=False)

    backend_name = 'dummy' if args.dry_run or args.backend == 'dummy' else (args.backend if args.backend != 'auto' else args.model)
    ...
    for _, row in selected.iterrows():
        ...
        if raw_path.exists() and not args.resume:
            raw_path.unlink()
        result = run_backend_with_retries(backend, prompt, image_paths, raw_path, parse_error_path, episode_id)
        record = {
        ...
        if processed % max(1, args.batch_size) == 0:
            ...
            print(f'processed={processed} total_selected={len(selected)} rate={processed/elapsed:.3f}/s episode={episode_id}', flush=True)
