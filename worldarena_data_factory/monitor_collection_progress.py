...
def target_count(job):
    return int(job.get("target_success") or 0)


def capped(job):
    return min(success_count(job), target_count(job))
...
def render(out: Path, jobs):
    ...
    cfg_done = defaultdict(int)
    cfg_total = defaultdict(int)
    for j in jobs:
        fam = j.get("task_family", "unknown")
        cfg = (j.get("base_task_config") or j.get("task_config", "")).split("__job_")[0]
        fam_done[fam] += capped(j)
        fam_total[fam] += target_count(j)
        cfg_done[cfg] += capped(j)
        cfg_total[cfg] += target_count(j)
    lines = []
    lines.append("WorldArena v0 Aloha-AgileX collection progress")
    lines.append(bar(done, total) + f" episodes {done}/{total}")
    lines.append(
        f"jobs complete={complete_jobs}/{len(jobs)} partial={partial_jobs} empty={empty_jobs}"
    )
    lines.append("")
    lines.append("By config:")
    for cfg in sorted(cfg_total):
        lines.append(
            f"  {cfg:32s} {cfg_done[cfg]:4d}/{cfg_total[cfg]:4d} {bar(cfg_done[cfg],cfg_total[cfg],20)}"
        )
    lines.append("")
    lines.append("By task family:")
    for fam in sorted(fam_total):
        lines.append(
            f"  {fam:28s} {fam_done[fam]:4d}/{fam_total[fam]:4d} {bar(fam_done[fam],fam_total[fam],20)}"
        )
    log = latest_log(out)
    if log:
